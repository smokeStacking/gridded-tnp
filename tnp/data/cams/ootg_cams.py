...
class CAMSOOTGDataGenerator(CAMSDataGenerator):
    def __init__(
        self,
...
class CAMSStationOOTGDataGenerator(CAMSOOTGDataGenerator, CAMSStationDataGenerator):
    def generate_batch(self, batch_shape: Optional[torch.Size] = None) -> Batch:
        assert self.data is not None, "Data has not been loaded. Cannot generate batch."
        batch_size = self.batch_size if batch_shape is None else batch_shape[0]
...
class CAMSOOTGDataGeneratorWithReset(
    CAMSOOTGDataGenerator, BaseCAMSDataGeneratorWithReset
):
    pass


class CAMSStationOOTGDataGeneratorWithReset(
    CAMSStationOOTGDataGenerator, BaseCAMSDataGeneratorWithReset
):
    pass
