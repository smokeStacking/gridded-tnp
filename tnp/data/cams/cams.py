...
from tnp.data.cams.normalisation import locations as var_means
from tnp.data.cams.normalisation import normalise_var
from tnp.data.cams.normalisation import scales as var_stds
...
@dataclass
class CAMSBatch(Batch, BaseCAMSBatch):
    time_grid: Optional[torch.Tensor] = None


class BaseCAMSDataGenerator(DataGenerator, ABC):
    def __init__(
        self,
        *,
        ...
        load_data: bool = True,
        data_dir: Optional[str] = "./data/cams",
            ...
            for fname in self.fnames:
                try:
                    ...
                    if 'valid_time' in dataset.coords:
                        ...
                        logging.debug("No time coordinate found in %s", fname)
                        continue
                    
                    dataset = dataset.sel({time_coord: slice(*sorted(time_range))})
                    if dataset[time_coord].size == 0:
...
class BaseCAMSDataGeneratorWithReset(BaseCAMSDataGenerator, ABC):
    def __init__(
        self,
        *,
        num_months: int = 1,
...
class CAMSDataGenerator(BaseCAMSDataGenerator):
    def __init__(
        self,
        *,
        min_pc: float,
...
class CAMSStationDataGenerator(CAMSDataGenerator):
    def __init__(
...
class CAMSDataGeneratorWithReset(CAMSDataGenerator, BaseCAMSDataGeneratorWithReset):
    pass


def coarsen_grid_cams(
    grid: torch.Tensor,
