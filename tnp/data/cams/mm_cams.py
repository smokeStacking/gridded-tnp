...
from tnp.data.cams.cams import (
    BaseCAMSBatch,
...
@dataclass
class CAMSMultiModalBatch(MultiModalBatch, BaseCAMSBatch, Batch):
    time_grid: Optional[torch.Tensor] = None


class MultiModalCAMSDataGenerator(CAMSDataGenerator):
    ...
    def sample_batch(
        ...
        print(f"MultiModalCAMSDataGenerator: (latent) time_grid[0]: {time_grid[0]} of shape {time_grid.shape}")
        print(f"MultiModalCAMSDataGenerator: x_grid shape: {x_grid.shape}")
        ...
        print(f"MultiModalCAMSDataGenerator: xt shape: {xt.shape}, yt shape: {yt.shape}")
...
class MultiModalCAMSDataGeneratorWithReset(
    MultiModalCAMSDataGenerator, BaseCAMSDataGeneratorWithReset
):
    pass
